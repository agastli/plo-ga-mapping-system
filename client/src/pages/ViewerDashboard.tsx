...
export default function ViewerDashboard() {
  ...
  const [sortKey, setSortKey] = useState<SortKey>(() => {
    try { return (localStorage.getItem('viewer_programs_sortKey') as SortKey) || 'name'; } catch { return 'name'; }
  });
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>(() => {
    try { return (localStorage.getItem('viewer_programs_sortDir') as 'asc' | 'desc') || 'asc'; } catch { return 'asc'; }
  });
  ...
  const handleSort = (key: SortKey) => {
    if (sortKey === key) {
      ...
      setSortDir(next);
      try { localStorage.setItem('viewer_programs_sortDir', next); } catch {}
    } else {
      setSortKey(key);
      setSortDir('asc');
      try { localStorage.setItem('viewer_programs_sortKey', key); localStorage.setItem('viewer_programs_sortDir', 'asc'); } catch {}
    }
  };
          ...
          <div className={`p-4 rounded-lg border-l-4 ${scopeInfo.bg} ${scopeInfo.border} shadow-sm`}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-1.5">You can view analytics and programs within these scopes.</p>
              </div>
            </div>
          </div>
        )}
